...
class CoreTest(JacazulTest):
    ...
    def test_tools_executable(self):
        ...
        for tool in [self.taskp, self.tw_flow, self.ponder]:
            self.assertTrue(
                os.access(tool, os.X_OK), f"Tool {tool} is not executable"
            )
    ...
    def test_taskdata_isolation(self):
        """Verify strict TASKDATA isolation: Alpha cannot see Beta."""
        # Setup Project Alpha
        ...
        self.assertNotIn(
            "Beta Task", out, "Isolation breach: foreign task leaked!"
        )
