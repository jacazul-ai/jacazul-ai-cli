...
class SecurityTest(JacazulTest):
    ...
    def test_task_description_quote_handling(self):
        """Edge Case: Ensure descriptions handle quotes correctly."""
    ...
    def test_ponder_excludes_archive_projects(self):
        """Ponder must not show projects ending in _archive."""
    ...
    def test_taskp_blocks_direct_done_command(self):
        """Vaccination: taskp MUST block 'done' to enforce tw-flow."""
        self.run_cmd(f"{self.taskp} add 'Sec task'")
        out, err, code = self.run_cmd(f"{self.taskp} 1 done")
        self.assertNotEqual(code, 0)
        self.assertIn(
            "ERROR: Direct 'done' command via taskp is restricted", out + err
        )
    ...
    def test_taskp_blocks_manual_discard_tag(self):
        ...
        self.assertNotEqual(code, 0)
        self.assertIn(
            "ERROR: Manual '+DISCARDED' tag via taskp is restricted", out + err
        )

    def test_discard_creates_automatic_outcome_audit(self):
        """Auditing: discard must automatically document task closure."""
        out, _, _ = self.run_cmd(
            f"{self.tw_flow} ini disc 'Discard me|r|today'"
        )
        ...
        self.assertIn(
            "OUTCOME: Task discarded and moved to archive", normalized_out
        )
