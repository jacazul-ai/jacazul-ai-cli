...
except its `evals/`, because the cases are written by hand and never
generated by the hatch.

Running a suite, writing graders that measure what the agent did, reading
traces and comparing before and after a change are covered in
[`skill-evals.md`](skill-evals.md).
