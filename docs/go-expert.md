...
| `golang-benchmark` | `references/benchmarks.md`, `references/profiling.md` | Split by question rather than by upstream file. Benchmarks own producing a number to compare; profiling owns finding where it is spent. A second pass read `pprof.md` in depth for interpretation rather than surface: flat against cum, runtime frames as symptoms, the heap pair read together and diffed with `-base`, labels, and the goroutine leak views, including the `goroutineleak` profile upstream does not cover. The flag catalogue, export formats and symbolization options stay with `go tool pprof -h`, which owns them and cannot drift. |
