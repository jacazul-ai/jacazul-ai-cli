...
task 42 annotate "RESEARCH: Found passport.js library - supports OAuth"
task 42 annotate "DECISION: Using JWT with 15min expiry for access tokens"
task 42 annotate "LINK: https://github.com/jaredhanson/passport"
```
...
- `depends:ID` - Blocks this task until task ID is done

**Priority System (USE THIS):**
- First task in plan: `urgency:9.0 due:today` (work on this NOW)
- Second task: `urgency:7.0 due:tomorrow depends:FIRST_ID` (blocked until first is done)
- Third task: `urgency:5.0 due:2days depends:SECOND_ID` (blocked until second is done)
- Fourth task: `urgency:3.0 due:3days depends:THIRD_ID` (blocked until third is done)
...
**Dependencies System:**
- `depends:ID` makes current task BLOCKED until task ID is completed
...
- Set dependencies with `depends:task_id` to block tasks until prerequisites are done
...
task <id> annotate "RESEARCH: Found X library that does Y"

# Decisions made (why you chose something)
task <id> annotate "DECISION: Chose approach A over B because of performance"

# Blockers (what's stopping progress)
task <id> annotate "BLOCKED: Waiting for PR #123 to merge"

# Lessons learned (gotchas, tips)
task <id> annotate "LESSON: Always test edge cases early"

# Acceptance criteria (what "done" means)
task <id> annotate "AC: Must handle 1000+ concurrent users"

# Notes (general observations)
task <id> annotate "NOTE: Customer prefers dark mode"

# Links (references)
task <id> annotate "LINK: https://docs.example.com/api"
```

**Example - Complete research task with annotations:**
```bash
task 42 start
task 42 annotate "RESEARCH: Evaluated 3 auth libraries: passport.js, jsonwebtoken, auth0"
task 42 annotate "DECISION: Using passport.js - most flexible for multi-strategy"
task 42 annotate "LINK: https://github.com/jaredhanson/passport"
task 42 annotate "AC: Must support OAuth, SAML, and local authentication"
task 42 done
```
...
task 42 modify urgency:10.0

# Lower priority
task 42 modify urgency:2.0

# Remove manual urgency (use automatic calculation)
task 42 modify urgency:
```

**Dependencies (Task Blocking):**
- Use `depends:ID` to block a task until another is completed
...
**Example output of `task 42 info`:**
```
...
task 42 annotate "RESEARCH: Evaluated 3 options: REST, GraphQL, gRPC"
task 42 annotate "DECISION: Using REST for simplicity"
...
task 10 start
task 10 annotate "RESEARCH: Comparing passport.js vs auth0"
task 10 annotate "DECISION: Using passport.js for flexibility"
task 10 annotate "AC: Must support OAuth and local auth"
task 10 done
# Task 11 is now UNBLOCKED!

# Step 3: Work on second task
task 11 start
task 11 annotate "IMPLEMENTATION: Using jsonwebtoken library"
task 11 annotate "NOTE: Access tokens expire in 15 minutes"
task 11 done
# Task 12 is now UNBLOCKED!

# Step 4: Work on third task
task 12 start
task 12 annotate "TESTING: Covering success and failure cases"
task 12 done
# Plan complete! ✅
```
...
task status:pending ready
# Shows task 30 at the top!

# Work on it immediately
task 30 start
task 30 annotate "BUG: Null pointer exception in auth middleware"
task 30 annotate "FIX: Added null check before token validation"
task 30 done
...
task <id> modify urgency:9.5

# Reset urgency to auto-calculated
task <id> modify urgency:
```
...
# Output:
# ✓ Created task 42: Design auth flow [urgency: 9.0]
# ✓ Created task 43: Implement JWT [urgency: 7.0]
# ✓ Created task 44: Write tests [urgency: 5.0]
...
# 2. Work on it
task 1 start
task 1 annotate "RESEARCH: Found library X"
task 1 done

# 3. Next task auto-unblocks
task 2 start
task 2 annotate "IMPLEMENTATION: Using approach Y"
task 2 done

# 4. Final task
task 3 start
task 3 annotate "TESTING: All cases pass"
task 3 done
```
...
# Lessons learned
task <id> annotate "LESSON: Testing edge cases earlier saves debugging time"
